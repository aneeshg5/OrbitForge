...
// GPS position-only sensor model.
// ECI position → ECEF via GAST rotation → additive Gaussian noise N(0, sigma_m²·I).
// Default sigma = 10 m; configurable 1 m (DGPS) to 100 m (degraded). See math.md §5.1.
class GpsSensor {
public:
    double sigma_m;  // position noise std dev [m]

    explicit GpsSensor(double sigma = 10.0, unsigned seed = 42)
        : sigma_m(sigma), rng_(seed), dist_(0.0, sigma)
    {}

    // Returns r_eci rotated to ECEF plus N(0, sigma_m²·I) noise.
    ...
    // 3×3 ECI→ECEF rotation matrix for use in filter Jacobians.
    ...
    static double gast_rad(double julian_date) {
        constexpr double deg_to_rad = 1.7453292519943295769e-2;  // π/180
